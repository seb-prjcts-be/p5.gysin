...
(function (global) {
  ...
  class GysinPlot {
    ...
    update(id, options = {}) {
      ...
      shape.human = normalizeHumanOptions(Object.assign({}, shape.human, split.human));
      shape.style = normalizeStyleOptions(Object.assign({}, shape.style, split.style));
      shape.exportSettings = normalizeExportOptions(Object.assign({}, shape.exportSettings, split.exportSettings));

      const paramKeys = Object.keys(options.params || {});
      for (const key of paramKeys) shape.params[key] = options.params[key];

      validateShapeParams(shape.type, shape.params);
      if (Object.prototype.hasOwnProperty.call(split.human, "seed")) {
        shape.baseSeed = shape.human.seed === null || shape.human.seed === undefined
    ...
    exportSVG(options = {}) {
      ...
      for (const [layer, layerTraces] of tracesByLayer(traces)) {
        ...
        for (const trace of layerTraces) {
          lines.push(`      <path d="${svgPathD(trace.points, decimals)}" stroke="${escapeXML(trace.style.stroke)}" stroke-width="${round(trace.style.strokeWeight, decimals)}" opacity="${round(trace.style.alpha, decimals)}" data-shape-id="${escapeXML(trace.shapeId)}" data-type="${escapeXML(trace.type)}" />`);
        }
        lines.push(`    </g>`);
      }
    ...
    stats(options = {}) {
      ...
      for (const trace of traces) {
        ...
        if (!layerStats[trace.layer]) layerStats[trace.layer] = { paths: 0, drawnLength: 0 };
        layerStats[trace.layer].paths += 1;
        layerStats[trace.layer].drawnLength += length;
      }
    ...
    _resolvePage(options = {}) {
      ...
      const fallbackWidth = this.width === null ? optionalPositiveNumber(global.width, "canvas width") : this.width;
      const fallbackHeight = this.height === null ? optionalPositiveNumber(global.height, "canvas height") : this.height;
  ...
  function normalizeHumanOptions(human) {
    return {
      ...
      repeat: positiveInteger(human.repeat, "repeat"),
  ...
  function validateShapeParams(type, params) {
    if (type === "line") {
      finiteNumber(params.x1, "line x1"); finiteNumber(params.y1, "line y1");
      finiteNumber(params.x2, "line x2"); finiteNumber(params.y2, "line y2");
      return;
    }
    if (type === "rect") {
      finiteNumber(params.x, "rect x"); finiteNumber(params.y, "rect y");
      finiteNumber(params.w, "rect width"); finiteNumber(params.h, "rect height");
      return;
    }
    if (type === "circle") {
      finiteNumber(params.x, "circle x"); finiteNumber(params.y, "circle y"); positiveNumber(params.diameter, "circle diameter");
      return;
    }
    if (type === "polygon") normalizePoints(params.points, "polygon", 3);
    if (type === "path") normalizePoints(params.points, "path", 2);
    if (type === "text" || type === "textCutup") {
      finiteNumber(params.x, `${type} x`); finiteNumber(params.y, `${type} y`);
      positiveNumber(params.size, "text size"); positiveNumber(params.sampleFactor, "sampleFactor");
      positiveNumber(params.lineHeight, "lineHeight"); finiteNumber(params.letterSpacing, "letterSpacing");
      if (type === "textCutup") {
        positiveInteger(params.slices, "slices"); finiteNumber(params.sliceOffset, "sliceOffset"); probability(params.sliceDropout, "sliceDropout");
      }
    }
  }
  ...
  function penForTrace(trace, penMap) {
    const pen = penMap[trace.layer] === undefined
      ? (penMap[trace.style.stroke] === undefined ? 1 : penMap[trace.style.stroke])
      : penMap[trace.layer];
