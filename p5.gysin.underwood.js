/*
 * p5.gysin.underwood
 * Optional, period-correct single-stroke typewriter for p5.gysin.
 *
 * Adds one intent verb: plot.underwood(text, x, y, options). Defaults alone give a
 * period-correct typewriter page - single-stroke letterforms, fixed monospace
 * pitch and line height (10 characters-per-inch / 6 lines-per-inch, the classic
 * Pica machine), and the small strike irregularity of a real typed sheet. Every
 * option is opt-in, so the one-liner is complete.
 *
 * Historical grain (see docs/typewriter-decoration-research.md): a typewriter
 * had no bold and no italic. The only "bold" is OVERSTRIKE - the same key
 * struck twice, a hair offset. Emphasis was underline, ALL CAPS, or spacing.
 * This module honours that: bold: true doubles every strike, underline draws
 * the underscore rule, and nothing here fakes a heavier or slanted typeface.
 *
 * The letterforms are Hershey "futural" (Sans 1-stroke), public domain, bundled
 * here so the verb is self-contained (no external font to load).
 *
 * Requires p5.gysin core (GysinPlot) loaded first.
 */
(function (global) {
  "use strict";

  // Hershey futural face. chars[i] is the glyph for ASCII code (i + 33); space
  // (32) is handled by the renderer. Each glyph: { d: "M.. L..", o: advance }.
  // d is in Hershey units: cap height ~21, baseline ~22, y increases downward.
  const FACE = [{"d":"M5,1 L5,15 M5,20 L4,21 5,22 6,21 5,20","o":5},{"d":"M4,1 L4,8 M12,1 L12,8","o":8},{"d":"M11,-3 L4,29 M17,-3 L10,29 M4,10 L18,10 M3,16 L17,16","o":11},{"d":"M8,-3 L8,26 M12,-3 L12,26 M17,4 L15,2 12,1 8,1 5,2 3,4 3,6 4,8 5,9 7,10 13,12 15,13 16,14 17,16 17,19 15,21 12,22 8,22 5,21 3,19","o":10},{"d":"M21,1 L3,22 M8,1 L10,3 10,5 9,7 7,8 5,8 3,6 3,4 4,2 6,1 8,1 10,2 13,3 16,3 19,2 21,1 M17,15 L15,16 14,18 14,20 16,22 18,22 20,21 21,19 21,17 19,15 17,15","o":12},{"d":"M23,10 L23,9 22,8 21,8 20,9 19,11 17,16 15,19 13,21 11,22 7,22 5,21 4,20 3,18 3,16 4,14 5,13 12,9 13,8 14,6 14,4 13,2 11,1 9,2 8,4 8,6 9,9 11,12 16,19 18,21 20,22 22,22 23,21 23,20","o":13},{"d":"M5,3 L4,2 5,1 6,2 6,4 5,6 4,7","o":5},{"d":"M11,-3 L9,-1 7,2 5,6 4,11 4,15 5,20 7,24 9,27 11,29","o":7},{"d":"M3,-3 L5,-1 7,2 9,6 10,11 10,15 9,20 7,24 5,27 3,29","o":7},{"d":"M8,7 L8,19 M3,10 L13,16 M13,10 L3,16","o":8},{"d":"M13,4 L13,22 M4,13 L22,13","o":13},{"d":"M5,18 L4,19 3,18 4,17 5,18 5,20 3,22","o":4},{"d":"M4,13 L22,13","o":13},{"d":"M4,17 L3,18 4,19 5,18 4,17","o":4},{"d":"M20,-3 L2,29","o":11},{"d":"M9,1 L6,2 4,5 3,10 3,13 4,18 6,21 9,22 11,22 14,21 16,18 17,13 17,10 16,5 14,2 11,1 9,1","o":10},{"d":"M6,5 L8,4 11,1 11,22","o":10},{"d":"M4,6 L4,5 5,3 6,2 8,1 12,1 14,2 15,3 16,5 16,7 15,9 13,12 3,22 17,22","o":10},{"d":"M5,1 L16,1 10,9 13,9 15,10 16,11 17,14 17,16 16,19 14,21 11,22 8,22 5,21 4,20 3,18","o":10},{"d":"M13,1 L3,15 18,15 M13,1 L13,22","o":10},{"d":"M15,1 L5,1 4,10 5,9 8,8 11,8 14,9 16,11 17,14 17,16 16,19 14,21 11,22 8,22 5,21 4,20 3,18","o":10},{"d":"M16,4 L15,2 12,1 10,1 7,2 5,5 4,10 4,15 5,19 7,21 10,22 11,22 14,21 16,19 17,16 17,15 16,12 14,10 11,9 10,9 7,10 5,12 4,15","o":10},{"d":"M17,1 L7,22 M3,1 L17,1","o":10},{"d":"M8,1 L5,2 4,4 4,6 5,8 7,9 11,10 14,11 16,13 17,15 17,18 16,20 15,21 12,22 8,22 5,21 4,20 3,18 3,15 4,13 6,11 9,10 13,9 15,8 16,6 16,4 15,2 12,1 8,1","o":10},{"d":"M16,8 L15,11 13,13 10,14 9,14 6,13 4,11 3,8 3,7 4,4 6,2 9,1 10,1 13,2 15,4 16,8 16,13 15,18 13,21 10,22 8,22 5,21 4,19","o":10},{"d":"M4,10 L3,11 4,12 5,11 4,10 M4,17 L3,18 4,19 5,18 4,17","o":4},{"d":"M4,10 L3,11 4,12 5,11 4,10 M5,18 L4,19 3,18 4,17 5,18 5,20 3,22","o":4},{"d":"M20,4 L4,13 20,22","o":12},{"d":"M4,10 L22,10 M4,16 L22,16","o":13},{"d":"M4,4 L20,13 4,22","o":12},{"d":"M3,6 L3,5 4,3 5,2 7,1 11,1 13,2 14,3 15,5 15,7 14,9 13,10 9,12 9,15 M9,20 L8,21 9,22 10,21 9,20","o":9},{"d":"M18,9 L17,7 15,6 12,6 10,7 9,8 8,11 8,14 9,16 11,17 14,17 16,16 17,14 M12,6 L10,8 9,11 9,14 10,16 11,17 M18,6 L17,14 17,16 19,17 21,17 23,15 24,12 24,10 23,7 22,5 20,3 18,2 15,1 12,1 9,2 7,3 5,5 4,7 3,10 3,13 4,16 5,18 7,20 9,21 12,22 15,22 18,21 20,20 21,19 M19,6 L18,14 18,16 19,17","o":14},{"d":"M9,1 L1,22 M9,1 L17,22 M4,15 L14,15","o":9},{"d":"M4,1 L4,22 M4,1 L13,1 16,2 17,3 18,5 18,7 17,9 16,10 13,11 M4,11 L13,11 16,12 17,13 18,15 18,18 17,20 16,21 13,22 4,22","o":10},{"d":"M18,6 L17,4 15,2 13,1 9,1 7,2 5,4 4,6 3,9 3,14 4,17 5,19 7,21 9,22 13,22 15,21 17,19 18,17","o":11},{"d":"M4,1 L4,22 M4,1 L11,1 14,2 16,4 17,6 18,9 18,14 17,17 16,19 14,21 11,22 4,22","o":10},{"d":"M4,1 L4,22 M4,1 L17,1 M4,11 L12,11 M4,22 L17,22","o":9},{"d":"M4,1 L4,22 M4,1 L17,1 M4,11 L12,11","o":8},{"d":"M18,6 L17,4 15,2 13,1 9,1 7,2 5,4 4,6 3,9 3,14 4,17 5,19 7,21 9,22 13,22 15,21 17,19 18,17 18,14 M13,14 L18,14","o":11},{"d":"M4,1 L4,22 M18,1 L18,22 M4,11 L18,11","o":11},{"d":"M4,1 L4,22","o":4},{"d":"M12,1 L12,17 11,20 10,21 8,22 6,22 4,21 3,20 2,17 2,15","o":8},{"d":"M4,1 L4,22 M18,1 L4,15 M9,10 L18,22","o":10},{"d":"M4,1 L4,22 M4,22 L16,22","o":7},{"d":"M4,1 L4,22 M4,1 L12,22 M20,1 L12,22 M20,1 L20,22","o":12},{"d":"M4,1 L4,22 M4,1 L18,22 M18,1 L18,22","o":11},{"d":"M9,1 L7,2 5,4 4,6 3,9 3,14 4,17 5,19 7,21 9,22 13,22 15,21 17,19 18,17 19,14 19,9 18,6 17,4 15,2 13,1 9,1","o":11},{"d":"M4,1 L4,22 M4,1 L13,1 16,2 17,3 18,5 18,8 17,10 16,11 13,12 4,12","o":10},{"d":"M9,1 L7,2 5,4 4,6 3,9 3,14 4,17 5,19 7,21 9,22 13,22 15,21 17,19 18,17 19,14 19,9 18,6 17,4 15,2 13,1 9,1 M12,18 L18,24","o":11},{"d":"M4,1 L4,22 M4,1 L13,1 16,2 17,3 18,5 18,7 17,9 16,10 13,11 4,11 M11,11 L18,22","o":10},{"d":"M17,4 L15,2 12,1 8,1 5,2 3,4 3,6 4,8 5,9 7,10 13,12 15,13 16,14 17,16 17,19 15,21 12,22 8,22 5,21 3,19","o":10},{"d":"M8,1 L8,22 M1,1 L15,1","o":8},{"d":"M4,1 L4,16 5,19 7,21 10,22 12,22 15,21 17,19 18,16 18,1","o":11},{"d":"M1,1 L9,22 M17,1 L9,22","o":9},{"d":"M2,1 L7,22 M12,1 L7,22 M12,1 L17,22 M22,1 L17,22","o":12},{"d":"M3,1 L17,22 M17,1 L3,22","o":10},{"d":"M1,1 L9,11 9,22 M17,1 L9,11","o":9},{"d":"M17,1 L3,22 M3,1 L17,1 M3,22 L17,22","o":10},{"d":"M4,-3 L4,29 M5,-3 L5,29 M4,-3 L11,-3 M4,29 L11,29","o":7},{"d":"M0,1 L14,25","o":7},{"d":"M9,-3 L9,29 M10,-3 L10,29 M3,-3 L10,-3 M3,29 L10,29","o":7},{"d":"M8,-1 L0,13 M8,-1 L16,13","o":8},{"d":"M0,29 L18,29","o":9},{"d":"M5,6 L3,8 3,10 4,11 5,10 4,9 3,10","o":4},{"d":"M15,8 L15,22 M15,11 L13,9 11,8 8,8 6,9 4,11 3,14 3,16 4,19 6,21 8,22 11,22 13,21 15,19","o":10},{"d":"M4,1 L4,22 M4,11 L6,9 8,8 11,8 13,9 15,11 16,14 16,16 15,19 13,21 11,22 8,22 6,21 4,19","o":9},{"d":"M15,11 L13,9 11,8 8,8 6,9 4,11 3,14 3,16 4,19 6,21 8,22 11,22 13,21 15,19","o":9},{"d":"M15,1 L15,22 M15,11 L13,9 11,8 8,8 6,9 4,11 3,14 3,16 4,19 6,21 8,22 11,22 13,21 15,19","o":10},{"d":"M3,14 L15,14 15,12 14,10 13,9 11,8 8,8 6,9 4,11 3,14 3,16 4,19 6,21 8,22 11,22 13,21 15,19","o":9},{"d":"M10,1 L8,1 6,2 5,5 5,22 M2,8 L9,8","o":7},{"d":"M15,8 L15,24 14,27 13,28 11,29 8,29 6,28 M15,11 L13,9 11,8 8,8 6,9 4,11 3,14 3,16 4,19 6,21 8,22 11,22 13,21 15,19","o":10},{"d":"M4,1 L4,22 M4,12 L7,9 9,8 12,8 14,9 15,12 15,22","o":10},{"d":"M3,1 L4,2 5,1 4,0 3,1 M4,8 L4,22","o":4},{"d":"M5,1 L6,2 7,1 6,0 5,1 M6,8 L6,25 5,28 3,29 1,29","o":5},{"d":"M4,1 L4,22 M14,8 L4,18 M8,14 L15,22","o":8},{"d":"M4,1 L4,22","o":4},{"d":"M4,8 L4,22 M4,12 L7,9 9,8 12,8 14,9 15,12 15,22 M15,12 L18,9 20,8 23,8 25,9 26,12 26,22","o":15},{"d":"M4,8 L4,22 M4,12 L7,9 9,8 12,8 14,9 15,12 15,22","o":10},{"d":"M8,8 L6,9 4,11 3,14 3,16 4,19 6,21 8,22 11,22 13,21 15,19 16,16 16,14 15,11 13,9 11,8 8,8","o":10},{"d":"M4,8 L4,29 M4,11 L6,9 8,8 11,8 13,9 15,11 16,14 16,16 15,19 13,21 11,22 8,22 6,21 4,19","o":9},{"d":"M15,8 L15,29 M15,11 L13,9 11,8 8,8 6,9 4,11 3,14 3,16 4,19 6,21 8,22 11,22 13,21 15,19","o":10},{"d":"M4,8 L4,22 M4,14 L5,11 7,9 9,8 12,8","o":6},{"d":"M14,11 L13,9 10,8 7,8 4,9 3,11 4,13 6,14 11,15 13,16 14,18 14,19 13,21 10,22 7,22 4,21 3,19","o":9},{"d":"M5,1 L5,18 6,21 8,22 10,22 M2,8 L9,8","o":7},{"d":"M4,8 L4,18 5,21 7,22 10,22 12,21 15,18 M15,8 L15,22","o":10},{"d":"M2,8 L8,22 M14,8 L8,22","o":8},{"d":"M3,8 L7,22 M11,8 L7,22 M11,8 L15,22 M19,8 L15,22","o":11},{"d":"M3,8 L14,22 M14,8 L3,22","o":9},{"d":"M2,8 L8,22 M14,8 L8,22 6,26 4,28 2,29 1,29","o":8},{"d":"M14,8 L3,22 M3,8 L14,8 M3,22 L14,22","o":9},{"d":"M9,-3 L7,-2 6,-1 5,1 5,3 6,5 7,6 8,8 8,10 6,12 M7,-2 L6,0 6,2 7,4 8,5 9,7 9,9 8,11 4,13 8,15 9,17 9,19 8,21 7,22 6,24 6,26 7,28 M6,14 L8,16 8,18 7,20 6,21 5,23 5,25 6,27 7,28 9,29","o":7},{"d":"M4,-3 L4,29","o":4},{"d":"M5,-3 L7,-2 8,-1 9,1 9,3 8,5 7,6 6,8 6,10 8,12 M7,-2 L8,0 8,2 7,4 6,5 5,7 5,9 6,11 10,13 6,15 5,17 5,19 6,21 7,22 8,24 8,26 7,28 M8,14 L6,16 6,18 7,20 8,21 9,23 9,25 8,27 7,28 5,29","o":7},{"d":"M3,16 L3,14 4,11 6,10 8,10 10,11 14,14 16,15 18,15 20,14 21,12 M3,14 L4,12 6,11 8,11 10,12 14,15 16,16 18,16 20,15 21,12 21,10","o":12},{"d":"M0,1 L0,22 1,22 1,1 2,1 2,22 3,22 3,1 4,1 4,22 5,22 5,1 6,1 6,22 7,22 7,1 8,1 8,22 9,22 9,1 10,1 10,22 11,22 11,1 12,1 12,22 13,22 13,1 14,1 14,22 15,22 15,1 16,1 16,22","o":8}];  /* __HERSHEY_DATA__ */

  const CAP_UNITS = 21;      // Hershey cap height in glyph units
  const BASELINE_UNITS = 22; // glyph y that sits on the baseline
  const DEFAULT_SIZE = 18;   // cap height in px

  class Rng {
    constructor(seedKey) {
      let h = 2166136261;
      const s = String(seedKey);
      for (let i = 0; i < s.length; i++) {
        h ^= s.charCodeAt(i);
        h = Math.imul(h, 16777619);
      }
      this.state = h >>> 0;
      if (this.state === 0) this.state = 0x6d2b79f5;
    }
    next() {
      let t = (this.state += 0x6d2b79f5);
      t = Math.imul(t ^ (t >>> 15), t | 1);
      t ^= t + Math.imul(t ^ (t >>> 7), t | 61);
      return ((t ^ (t >>> 14)) >>> 0) / 4294967296;
    }
    range(a, b) {
      return a + (b - a) * this.next();
    }
  }

  // Parse a glyph's "d" into polylines: subpaths split on M, points on L/space.
  function glyphPolylines(d) {
    return String(d).split("M")
      .map(function (chunk) { return chunk.trim(); })
      .filter(function (chunk) { return chunk.length; })
      .map(function (chunk) {
        const nums = chunk.replace(/L/g, " ").match(/-?\d+/g) || [];
        const pts = [];
        for (let i = 0; i + 1 < nums.length; i += 2) {
          pts.push([Number(nums[i]), Number(nums[i + 1])]);
        }
        return pts;
      })
      .filter(function (pts) { return pts.length >= 2; });
}

  function glyphFor(char) {
    const code = char.charCodeAt(0);
    if (code < 33 || code > 127) return null;
    return FACE[code - 33] || null;
  }

  function finite(value, name) {
    const n = Number(value);
    if (!Number.isFinite(n)) throw new TypeError(`underwood() ${name} must be a finite number.`);
    return n;
  }

  // The whole verb. Types text at (x, y) as single-stroke monospace, one pen
  // line per stroke, and returns the ids of every shape it creates.
  function typeText(plot, text, x, y, options) {
    if (!plot || typeof plot.path !== "function") {
      throw new TypeError("underwood() needs a GysinPlot instance.");
    }
    const o = options || {};
    finite(x, "x");
    finite(y, "y");

    const size = o.size === undefined ? DEFAULT_SIZE : positive(o.size, "size");
    // Period metrics: Pica = 10 chars/inch, 6 lines/inch single spacing. Pitch
    // sits a touch wider than the cap so wide letters (M, W) never touch; line
    // height follows the 6lpi/10cpi ratio, lightly tightened for dense blocks.
    const pitch = o.pitch === undefined ? size * 1.2 : positive(o.pitch, "pitch");
    const lineHeight = o.lineHeight === undefined ? size * 1.9 : positive(o.lineHeight, "lineHeight");
    const wear = o.wear === undefined ? 1 : nonNegative(o.wear, "wear");
    const bold = o.bold === true;
    const underline = o.underline ? Math.max(0, Math.min(3, Math.round(o.underline))) : 0;
    const scale = size / CAP_UNITS;

    // One knob, wear, scales every imperfection together (0 = mechanically clean).
    const wobble = 0.45 * wear;
    const baselineJitter = 0.7 * wear;
    const rotate = 0.025 * wear;
    const inkVary = 0.28 * wear;
    const overstrikeChance = bold ? 1 : 0.18 * wear;
    const dropout = 0.02 * wear;
    const weight = o.strokeWeight === undefined ? 1.1 : positive(o.strokeWeight, "strokeWeight");

    const seedKey = o.seed === undefined
      ? `${plot.globalSeed}:type:${x}:${y}:${text}`
      : o.seed;
    const rng = new Rng(seedKey);

    const ids = [];
    const lines = String(text).split("\n");

    for (let li = 0; li < lines.length; li++) {
      const line = lines[li];
      const baseY = y + li * lineHeight;

      for (let i = 0; i < line.length; i++) {
        const char = line[i];
        if (char === " ") continue;
        const glyph = glyphFor(char);
        if (!glyph) continue;
        const polys = glyphPolylines(glyph.d);
        if (!polys.length) continue;

        let minX = Infinity;
        let maxX = -Infinity;
        for (const poly of polys) {
          for (const p of poly) {
            if (p[0] < minX) minX = p[0];
            if (p[0] > maxX) maxX = p[0];
          }
        }
        const gcx = (minX + maxX) / 2;
        const cellCenter = x + i * pitch + pitch / 2;

        // Per-glyph strike: a hair high/low and a touch crooked, like a worn slug.
        const jy = rng.range(-baselineJitter, baselineJitter);
        const jx = rng.range(-0.4 * wear, 0.4 * wear);
        const rot = rng.range(-rotate, rotate);
        const cos = Math.cos(rot);
        const sin = Math.sin(rot);
        const ink = 1 - rng.next() * inkVary;

        const place = function (poly, dx, dy) {
          return poly.map(function (p) {
            const lx = (p[0] - gcx) * scale;
            const ly = (p[1] - BASELINE_UNITS) * scale;
            return [
              cellCenter + (lx * cos - ly * sin) + jx + dx,
              baseY + (lx * sin + ly * cos) + jy + dy
            ];
          });
        };

        const baseOpts = {
          wobble: wobble,
          segmentLength: 3,
          dropout: dropout,
          strokeWeight: weight * (0.85 + 0.15 * ink)
        };
        if (o.stroke !== undefined) baseOpts.stroke = o.stroke;
        if (o.layer !== undefined) baseOpts.layer = o.layer;
        baseOpts.alpha = 0.6 + 0.4 * ink;
        for (const poly of polys) ids.push(plot.path(place(poly, 0, 0), baseOpts));

        // Double-struck: the only period "bold". bold:true forces every glyph;
        // otherwise it happens in a few worn places.
        if (rng.next() < overstrikeChance) {
          const dx = rng.range(-0.5, 1.1);
          const dy = rng.range(-0.4, 0.7);
          const overOpts = {
            wobble: wobble * 1.2 + 0.1,
            segmentLength: 3,
            strokeWeight: weight * 0.95,
            alpha: 0.8
          };
          if (o.stroke !== undefined) overOpts.stroke = o.stroke;
          if (o.layer !== undefined) overOpts.layer = o.layer;
          for (const poly of polys) ids.push(plot.path(place(poly, dx, dy), overOpts));
        }
      }

      // Underline: the underscore rule. 1 = italic, 2 = bold/small-caps,
      // 3 = caps, in the period proof-mark ladder. Drawn as single-stroke rules.
      if (underline > 0 && line.trim().length) {
        const lineWidth = line.replace(/\s+$/, "").length * pitch;
        for (let u = 0; u < underline; u++) {
          const uy = baseY + size * 0.28 + u * size * 0.14;
          const ruleOpts = {
            wobble: wobble * 0.6,
            strokeWeight: weight
          };
          if (o.stroke !== undefined) ruleOpts.stroke = o.stroke;
          if (o.layer !== undefined) ruleOpts.layer = o.layer;
          ids.push(plot.line(x + pitch * 0.15, uy, x + lineWidth - pitch * 0.15, uy, ruleOpts));
        }
      }
    }

    return ids;
  }

  function positive(value, name) {
    const n = Number(value);
    if (!Number.isFinite(n) || n <= 0) throw new RangeError(`underwood() ${name} must be greater than zero.`);
    return n;
  }
  function nonNegative(value, name) {
    const n = Number(value);
    if (!Number.isFinite(n) || n < 0) throw new RangeError(`underwood() ${name} must be zero or greater.`);
    return n;
  }

  const GysinUnderwood = Object.freeze({
    face: FACE,
    glyphPolylines: glyphPolylines,
    render: typeText
  });
  global.GysinUnderwood = GysinUnderwood;

  // Register the intent verb on the core, if it is present.
  if (global.GysinPlot && global.GysinPlot.prototype && !global.GysinPlot.prototype.underwood) {
    global.GysinPlot.prototype.underwood = function (text, x, y, options) {
      return typeText(this, text, x, y, options);
    };
  }
})(typeof globalThis !== "undefined" ? globalThis : window);
